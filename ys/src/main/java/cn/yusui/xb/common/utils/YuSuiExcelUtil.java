 ...
 * @author linjingfeng
 */
public class YuSuiExcelUtil<T> {
    Class<T> clazz;

    public YuSuiExcelUtil(Class<T> clazz) {
        this.clazz = clazz;
    }
        ...
        try {
            ...
            if (rows > 0) {// 有数据时才处理
                ...
                for (Field field : allFields) {
                    // 将有注解的field存放到map中.
                    if (field.isAnnotationPresent(YuSuiExcelParam.class)) {
                        YuSuiExcelParam attr = field
                                .getAnnotation(YuSuiExcelParam.class);
                        ...
                        if (cellType == HSSFCell.CELL_TYPE_NUMERIC) {
                            ...
                            if (HSSFDateUtil.isCellDateFormatted(cell)) {
                               ...
                               /* short format = cell.getCellStyle().getDataFormat();*/
                                ...
                                double cnum = cell.getNumericCellValue();
                                NumberFormat nf = NumberFormat.getInstance();
                                String s = nf.format(cnum);
                                if (s.indexOf(",") >= 0) {
                                    //这种方法对于自动加".0"的数字可直接解决
                                    // 但如果是科学计数法的数字就转换成了带逗号的，例如：12345678912345的科学计数法是1.23457E+13，经过这个格式化后就变成了字符串“12,345,678,912,345”，
                                    // 这也并不是想要的结果，所以要将逗号去掉
                                    s = s.replace(",", "");
                                }
                                c = String.valueOf(s);
            ...
            for (int i = startNo; i < endNo; i++) {
                ...
                for (int j = 0; j < fields.size(); j++) {
                    ...
                    YuSuiExcelParam attr = field
                            .getAnnotation(YuSuiExcelParam.class);
                    try {
                        // 根据YuSuiExcelParam中设置情况决定是否导出,有些情况需要保持为空,希望用户填写这一列.
                        if (attr.isExport()) {
